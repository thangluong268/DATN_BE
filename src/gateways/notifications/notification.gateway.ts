...
export class NotificationGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  async joinRoom(@ConnectedSocket() client: AuthSocket) {
    ...
    this.io.emit(WS_EVENT.NOTIFICATION.JOIN_ROOM_NOTIFICATION, `User ${client.userId} joined rom`);
  }

  async sendNotification(receiverId: string, body: NotificationGetRESP) {
    this.logger.log(`Send notification}`);
    this.io.to(receiverId).emit(WS_EVENT.NOTIFICATION.SEND_NOTIFICATION, body);
  }
  ...
  async sendAddFriendNotification(@ConnectedSocket() client: AuthSocket, body: NotificationCreateREQ) {
    ...
    this.io.to(receiverId).emit(WS_EVENT.NOTIFICATION.SENT_ADD_FRIEND_NOTIFICATION, newNotification);
  }
